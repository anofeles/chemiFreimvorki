        ...
        <div id="sidebar-menu">
            ...
            foreach ($viu->data[0]['grant'] as$datum) {
                ...
                foreach ($viu->data[0]['grname'] as $gr =>  $grtum) {
                    $werarq->grantarq($_SESSION['userinfo']['PersCode']);
                    if(isset($werarq->grantarq[$gr])){
                    if($werarq->grantarq[$gr]["grdas"] == 0) {
                        if($datum['grid'] == $grtum['GrantID']) {
                            ...
                            foreach ($viu->data[0]['werval'] as $g => $werval) {
                                if($datum['grid'] == $werval['grid']) {

                                    $werarq->teqarq($werval['grid'], $werval['id']);
                                    if(isset($werarq->grvtitle[$g]["grdas"])) {
                                        echo "<li class='has_sub'>
                            <a href='javascript:void(0);' class='waves-effect'><span>" . $werval['workerName'] . "</span> <span class='menu-arrow'></span></a><ul style=''>";
                                        foreach ($viu->data[0]['grvtitle'] as $grvtitle) {
                                            if($grvtitle['grantID'] == $werval['grid'] && $grvtitle['werID'] == $werval['id']) {
                                                if($grvtitle['grdas'] == 0) {
                                                    echo "<li><a href='" . $viu->urlbase . "/home/shestan/" . $grvtitle['grantID'] . "/" . $grvtitle['werID'] . "/?red=" . $grvtitle['kontaqt'] . "&incode=" . $grvtitle['invCode'] . "'><span>" . $grvtitle['title'] . "</span></a></li>";
                                                }
                                            }
                                        }
                                        echo "</ul></li>";
...
<br><input type='submit' value='მიწერა'>
                ...
                echo "<form action='#' method='post'>
<input type='submit' name='grgagz' value='გრანტის დასრულება'>
<input type='hidden' value='".$_GET['red']."' name='teqid'>
</form>";
            }
