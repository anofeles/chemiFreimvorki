...
class shetan extends dataase
{
    ...
    {
        $query = $this->mySQL->query("SELECT grid FROM grweruser WHERE perscode = $pc AND grdas = 0 GROUP BY grid");
...
WHERE grweruser.perscode = $pcode");
        while ($row = $query->fetch()) {
            $this->grantarq[] = $row;
        }
